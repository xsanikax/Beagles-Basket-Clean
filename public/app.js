...
let pickerTimer;let pickerRequest=0;let pickerResults=[];let pickerQuery="";let pickerHistoryActive=false;let pickerSort="relevance";let pickerBrand="all";
...
function sortPickerProducts(products,selected,bought){
  return products.map((product,index)=>({product,index})).sort((a,b)=>{
    if(pickerSort==="price-low")return Number(a.product.price)-Number(b.product.price)||a.index-b.index;
    if(pickerSort==="price-high")return Number(b.product.price)-Number(a.product.price)||a.index-b.index;
    if(pickerSort==="own-brand")return Number(!/^morrisons\b/i.test(a.product.name))-Number(!/^morrisons\b/i.test(b.product.name))||Number(a.product.price)-Number(b.product.price)||a.index-b.index;
