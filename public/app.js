...
function addItem(raw,chosenProduct=null){
  ...
  if(existing){
    if(chosenProduct){state.productSelections.morrisons[key]=chosenProduct.name;state.prices.morrisons[key]=chosenProduct.price;state.priceSources.morrisons[key]={productName:chosenProduct.name,size:chosenProduct.size,updatedAt:Date.now(),sourceUrl:cached?.sourceUrl||`https://groceries.morrisons.com/search?q=${encodeURIComponent(name)}`,options:cached?.options||pickerResults,resultVersion:cached?.resultVersion||4,imageVersion:cached?.imageVersion||2};rememberProducts(cached?.options||pickerResults,Date.now(),state.priceSources.morrisons[key].sourceUrl);}
...
let pickerTimer;let pickerRequest=0;let pickerResults=[];let pickerQuery="";let pickerHistoryActive=false;let pickerSort="relevance";let pickerBrand="all";let morrisonsCatalog=null;let morrisonsCatalogPromise=null;
...
function searchScore(query,product,index=0){const phrase=normalize(query).replace(/[^a-z0-9 ]+/g," ").trim();const tokens=phrase.split(" ").filter(token=>token.length>1);const name=normalize(product.name).replace(/[^a-z0-9 ]+/g," ");const coverage=tokens.filter(token=>name.includes(token)).length/Math.max(tokens.length,1);return (name.includes(phrase)?40:0)+coverage*30-Number(product.sitemapOnly)*3-index*.001;}
async function loadMorrisonsCatalog(){if(morrisonsCatalog)return morrisonsCatalog;if(!morrisonsCatalogPromise)morrisonsCatalogPromise=fetch("/morrisons-catalog.json",{cache:"force-cache"}).then(response=>response.ok?response.json():null).then(data=>morrisonsCatalog=Array.isArray(data?.products)?data.products:[]).catch(()=>morrisonsCatalog=[]);return morrisonsCatalogPromise;}
function localCatalogMatches(query,limit=220){if(!morrisonsCatalog?.length)return [];return morrisonsCatalog.map((product,index)=>({...product,score:searchScore(query,product,index)})).filter(product=>product.score>0).sort((a,b)=>b.score-a.score).slice(0,limit).map(({score,...product})=>product);}
function mergeProducts(...groups){const seen=new Set();return groups.flat().filter(product=>{const key=product.retailerProductId||normalize(product.name);if(!key||seen.has(key))return false;seen.add(key);return true;});}
function productBrand(product){const name=product.name||"";if(/^morrisons\b/i.test(name))return "Morrisons";return name.split(/\s+/).slice(0,2).join(" ").replace(/[^\w&'. -]/g,"").trim()||"Other";}
function productPrice(product){const price=Number(product.price);return Number.isFinite(price)?price:null;}
function sortPickerProducts(products,selected,bought){
  return products.map((product,index)=>({product,index})).sort((a,b)=>{
    const ap=productPrice(a.product),bp=productPrice(b.product);
    if(pickerSort==="price-low")return Number(ap===null)-Number(bp===null)||(ap??0)-(bp??0)||a.index-b.index;
    if(pickerSort==="price-high")return Number(ap===null)-Number(bp===null)||(bp??0)-(ap??0)||a.index-b.index;
    if(pickerSort==="own-brand")return Number(!/^morrisons\b/i.test(a.product.name))-Number(!/^morrisons\b/i.test(b.product.name))||Number(ap===null)-Number(bp===null)||(ap??0)-(bp??0)||a.index-b.index;
    return Number(b.product.name===selected)-Number(a.product.name===selected)||(bought[b.product.name]||0)-(bought[a.product.name]||0)||Number(!/^morrisons\b/i.test(a.product.name))-Number(!/^morrisons\b/i.test(b.product.name))||a.index-b.index;
  }).map(entry=>entry.product);
}
function renderProductPicker(query,products){
  pickerQuery=query;const selected=state.productSelections.morrisons[normalize(query)];const fallback=infoFor(query)[1];const bought=state.history.reduce((counts,event)=>{if(event.productName)counts[event.productName]=(counts[event.productName]||0)+1;return counts},{});const brands=[...new Set(products.map(productBrand))].sort((a,b)=>a.localeCompare(b));if(pickerBrand!=="all"&&!brands.includes(pickerBrand))pickerBrand="all";const filtered=pickerBrand==="all"?products:products.filter(product=>productBrand(product)===pickerBrand);pickerResults=sortPickerProducts(filtered,selected,bought);$("#picker-title").textContent=`Choose your ${query} · ${pickerResults.length} of ${products.length} matches`;$("#picker-title").nextElementSibling.textContent=products.length>60?"Filter and sort the wider Morrisons result set":"Remembered for the next quest";$("#product-options").innerHTML=`<div class="picker-tools"><label>Sort <select id="picker-sort"><option value="relevance">Best match</option><option value="price-low">Price low to high</option><option value="price-high">Price high to low</option><option value="own-brand">Morrisons own first</option></select></label><label>Brand <select id="picker-brand"><option value="all">All brands</option>${brands.map(brand=>`<option value="${escapeHtml(brand)}">${escapeHtml(brand)}</option>`).join("")}</select></label></div>${pickerResults.length?pickerResults.map((product,index)=>`<button type="button" role="option" aria-selected="${product.name===selected}" class="product-option ${product.name===selected?"active":""}" data-picker-index="${index}"><span class="product-thumb-wrap"><span>${fallback}</span>${product.image?`<img class="product-thumb" src="/api/morrisons/image?url=${encodeURIComponent(product.image)}" alt="" loading="lazy" onerror="this.remove()">`:""}</span><span class="product-copy"><strong>${escapeHtml(product.name)}${/^morrisons\b/i.test(product.name)?'<span class="own-brand">Morrisons own</span>':""}</strong><small>${escapeHtml(product.size||"Pack size unavailable")} · ${escapeHtml(productBrand(product))}${product.name===selected?" · Your usual":bought[product.name]?` · Bought ${bought[product.name]}×`:""}</small></span><b>${productPrice(product)===null?"Price pending":money(productPrice(product))}</b></button>`).join(""):'<div class="picker-empty">No products match this filter.</div>'}`;$("#picker-sort").value=pickerSort;$("#picker-brand").value=pickerBrand;showProductPicker();
}
async function findProductsForInput(raw){
  ...
  const savedSource=state.priceSources.morrisons[normalize(query)];const cached=savedSource?.options;if(savedSource?.resultVersion>=4&&savedSource?.imageVersion>=2&&cached?.length>=60){renderProductPicker(query,cached);return;}
  const request=++pickerRequest;pickerQuery=query;$("#product-options").innerHTML='<div class="picker-loading">Loading local Morrisons catalogue…</div>';showProductPicker();
  const catalog=await loadMorrisonsCatalog();if(request!==pickerRequest)return;const localMatches=localCatalogMatches(query);if(localMatches.length)renderProductPicker(query,localMatches);else{$("#product-options").innerHTML='<div class="picker-loading">Checking Morrisons…</div>';showProductPicker();}
  try{const response=await fetch("/api/morrisons/prices",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({items:[query]})});const data=await response.json();if(request!==pickerRequest)return;const result=data.results?.[0];if(!result?.ok||!result.products?.length)throw new Error("No matches");const merged=mergeProducts(result.products,localMatches);rememberProducts(merged,result.updatedAt,result.sourceUrl);state.priceSources.morrisons[normalize(query)]={productName:savedSource?.productName||"",size:savedSource?.size||"",updatedAt:result.updatedAt,sourceUrl:result.sourceUrl,options:merged,resultVersion:4,imageVersion:2};saveLocal();afterLocalAction("mergePriceData",{prices:state.prices,priceSources:state.priceSources,productCatalog:state.productCatalog,productSelections:state.productSelections,lastMorrisonsRefresh:state.lastMorrisonsRefresh||null,lastMorrisonsError:state.lastMorrisonsError||null});renderProductPicker(query,merged);}catch(error){if(request===pickerRequest&&!localMatches.length){$("#product-options").innerHTML='<div class="picker-empty">Morrisons search is unavailable. Try Refresh prices.</div>';showProductPicker();}}
}
...
$("#product-options").addEventListener("change",e=>{if(e.target.id==="picker-sort"){pickerSort=e.target.value;renderProductPicker(pickerQuery,state.priceSources.morrisons[normalize(pickerQuery)]?.options||pickerResults);return}if(e.target.id==="picker-brand"){pickerBrand=e.target.value;renderProductPicker(pickerQuery,state.priceSources.morrisons[normalize(pickerQuery)]?.options||pickerResults);}});
...
document.addEventListener("change",e=>{if(!e.target.matches(".check"))return;const item=state.items.find(i=>i.id===e.target.dataset.id);if(!item)return;if(!e.target.checked){item.done=false;delete item.completedAt;afterLocalAction("setDone",{id:item.id,done:false});render();return;}const row=e.target.closest(".item");e.target.disabled=true;row?.classList.add("is-completing");const reducedMotion=globalThis.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches;const delay=reducedMotion?0:560;setTimeout(()=>{const current=state.items.find(i=>i.id===item.id);if(!current)return;current.done=true;current.completedAt=Date.now();recentlyMovedItemId=current.id;afterLocalAction("setDone",{id:current.id,done:true,completedAt:current.completedAt});render();setTimeout(()=>{recentlyMovedItemId=null},reducedMotion?0:480)},delay)});
