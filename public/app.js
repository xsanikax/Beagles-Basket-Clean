...
document.addEventListener("change",e=>{if(!e.target.matches(".check"))return;const item=state.items.find(i=>i.id===e.target.dataset.id);if(!item)return;if(!e.target.checked){item.done=false;afterLocalAction("setDone",{id:item.id,done:false});render();return;}const row=e.target.closest(".item");e.target.disabled=true;row?.classList.add("is-completing");const delay=globalThis.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches?0:240;setTimeout(()=>{const current=state.items.find(i=>i.id===item.id);if(!current)return;current.done=true;recentlyMovedItemId=current.id;afterLocalAction("setDone",{id:current.id,done:true});render();setTimeout(()=>{recentlyMovedItemId=null},320)},delay)});
